...
router.get('/', function (req, res, next) {
    if (req.session.memberData) {
        var options = {
            method: 'GET',
            uri: environmentConfig.ApiServer + '/artwork/discover',
            headers: {
        ...
        rp(options)
            .then(function (json) {
                ...
                res.render('art-discover', { title: 'Look Lateral Test Discover Art', ApiServer: environmentConfig.ApiServer, MemberData: req.session.memberData, ErrorText: req.flash('error'), Artworks: artworks });
            })
            .catch(function (err) {
                // GET failed...
                console.log(err);
