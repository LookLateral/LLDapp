/* Test POST to Taurus API for artwork image publish. */
...
router.post('/art-publish-image-post', function (req, res, next) {
    if (req.session.memberData) {
        if (req.files) {
            var options = {
                method: 'POST',
                uri: environmentConfig.ApiServer + '/artwork/' + req.body.ArtworkId + '/uploadImage',
                body: {
            ...
            res.redirect(functions.redirectWithError(req, 'No image file was uploaded'));
        }
    } else {
        res.redirect('/login');
    }
